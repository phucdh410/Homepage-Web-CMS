...
import { deleteCategory, getCategories } from '@/apis/categories.api';
import { confirm } from '@/confirm/';
import { CSearchInput } from '@/controls/';
import { useNavigateQuery, useRevertQuery } from '@/hooks/';
import { CPagination } from '@/others/';
import { IGetCategoriesResponse } from '@/types/footer';

import { MCategoriesTable } from '../../components';
...
const ListCategoriesPage = () => {
  ...
  const listData = useMemo<IGetCategoriesResponse[]>(
    () => data?.data?.data?.data || [],
    [data],
  );
  //#endregion
  ...
  const onDelete = async (id: string) => {
    if (
      ...
      try {
        ...
        toast.error(
          error?.response?.data?.message || 'Xóa category không thành công!',
        );
      }
    }
  };
