...
import { deleteDepartment, getDepartments } from '@/apis/departments.api';
...
import { MDepartmentsTable } from '../../components';
...
const ListDepartmentsPage = () => {
  ...
  const listData = useMemo<IGetDepartmentsResponse[]>(
    () => data?.data?.data?.data || [],
    [data],
  );
  //#endregion
  ...
  const onDelete = (id: string) => async () => {
    if (
      ...
      try {
        ...
        toast.error(
          error?.response?.data?.message || 'Xóa phòng ban không thành công!',
        );
      }
    }
  };
