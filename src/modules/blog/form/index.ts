...
export const blogResolver: Resolver<ICreateOrUpdateBlogParams> = yupResolver(
  object({
    ...
    title: object({
      vi: string().required('Vui lòng nhập tiêu đề tiếng Việt!'),
      en: string().required('Vui lòng nhập tiêu đề tiếng Anh!'),
    }),
    description: object({
      vi: string().required('Vui lòng nhập mô tả tiếng Việt!'),
      en: string().required('Vui lòng nhập mô tả tiếng Anh!'),
    }),
    content: object({
